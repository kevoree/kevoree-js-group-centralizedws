...
var CentralizedWSGroup = AbstractGroup.extend({
	...
	start: function (done) {
		...
		if (rMasterNet && rMasterNet.length > 0) {
			if (isMaster) {
				this.server = server.create(logger(this.log, '['+this.name+'][master]'), port, this);
				done();
			} else {
				try {
					this.client = client.create(logger(this.log, '['+this.name+'][client]'), port, this, rMasterNet[1], rMasterNet[2]);
					done();
				} catch (err) {
					done(err);
				}
