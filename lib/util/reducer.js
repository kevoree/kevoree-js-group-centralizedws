...
function getRootPkgName(model, elem) {
	return /^\/packages\[([\w]+)\]/.exec(elem.typeDefinition.path())[1];
}

function getRelatedRootPackages(model, node) {
	var packages = [];

	packages.push(getRootPkgName(model, node));

	node.components.array.forEach(function (comp) {
		packages.push(getRootPkgName(model, comp));
	});
	...
	node.groups.array.forEach(function (group) {
		packages.push(getRootPkgName(model, group));
	});
...
function reducePackages(model, master, client) {
	...
	if (masterNode) {
		usedPackages = usedPackages.concat(getRelatedRootPackages(model, masterNode));
	}
	if (clientNode) {
		usedPackages = usedPackages.concat(getRelatedRootPackages(model, clientNode));
	}
